...
export default {
  ...
  get xpidlData() {
    ...
    if (process.env.TASK_ID && !process.env.MOZ_XPT_ARTIFACTS_DIR) {
      throw new Error("MOZ_XPT_ARTIFACTS_DIR must be set for this rule in automation");
    }
    ...
    for (let file of files) {
      ...
      let data = JSON.parse(fs.readFileSync(path.join(`${xpidlDir}`, file), { encoding: "utf-8" }));
  ...
  parseCode(filename, sourceText, astOptions = {}) {
    ...
    if (filename.endsWith(".mjs") || filename.endsWith(".ts") || sourceText.includes("import ")) {
      sourceType = "module";
    }

    // Use a permissive config file to allow parsing of anything that Espree
    // can parse.
    let config = { ...this.getPermissiveConfig(), ...astOptions, sourceType };
    ...
    config.ecmaVersion = config.ecmaVersion == "latest" ? 1e8 : config.ecmaVersion;
  ...
  getASTSource(node, context) {
    switch (node.type) {
      ...
      case "BinaryExpression":
        return (
          this.getASTSource(node.left) + " " + node.operator + " " + this.getASTSource(node.right)
        );
  ...
  getTestHeadFiles(scope) {
    ...
    let names = fs
      ...
      .filter(
        (name) =>
          (name.startsWith("head") || name.startsWith("xpcshell-head")) && name.endsWith(".js")
      )
    ...
    for (let name of names) {
      if (name.endsWith(".toml")) {
        try {
          ...
          console.error(
            "TOML ERROR: " + e.message + " @line: " + e.lineNumber + ", column: " + e.column
          );
        }
      }
    }
      ...
      function searchUpForPackage(dirName) {
        ...
        while (parsed.root !== dirName) {
          ...
          if (fs.existsSync(possibleFile)) {
            try {
              let packageData = JSON.parse(fs.readFileSync(possibleFile, { encoding: "utf-8" }));
  ...
  get globalScriptPaths() {
    return [
      ...
      path.join(this.rootDir, "browser", "base", "content", "global-scripts.js"),
    ];
  },
  ...
  getBuildEnvironment() {
    var output = execFileSync(path.join(this.rootDir, "mach"), ["environment", "--format=json"], {
      encoding: "utf-8",
    });
