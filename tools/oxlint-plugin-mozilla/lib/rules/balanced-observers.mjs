/**
 * @file Check that there's a Services.(prefs|obs).removeObserver for each addObserver.
 * @license
...
export default {
  meta: {
    ...
    messages: {
      noCorresponding: "No corresponding 'removeObserver(\"{{observable}}\")' was found.",
    },
    schema: [],
    type: "problem",
  },
  ...
  create(context) {
    ...
    function getObservableName(node) {
      ...
      const observableArgument = api === "obs" ? node.arguments[1] : node.arguments[0];
    ...
    function getUnbalancedObservers() {
      const unbalanced = addedObservers.filter((observer) => !hasRemovedObserver(observer));
    ...
    function hasRemovedObserver(addedObserver) {
      return removedObservers.some((observer) => addedObserver.observable === observer.observable);
    }
