...
function replaceConcatWithComma(fixer, node) {
  ...
  if (isConcatenation(node.left)) {
    ({ fixes: recursiveFixes, trailingIdentifier } = replaceConcatWithComma(fixer, node.left));
  ...
  if (
    ...
    fixes.push(fixer.replaceTextRange([node.left.range[1], node.right.range[0]], ", "));
          ...
          context.report({
            ...
            fix: (fixer) => {
              ...
              if (
                ...
                let { fixes: recursiveFixes } = replaceConcatWithComma(fixer, node.arguments[0]);
                fixes.push(...recursiveFixes);
              }
              return fixes;
            },
