...
let eslintBasePath = path.dirname(url.fileURLToPath(import.meta.resolve("eslint")));
...
const baseRule = (await import(getFileURLString("rules/no-redeclare.js"))).default;
...
function trapReport(context) {
  return function (obj) {
    ...
    while (declarator && declarator.parent && declarator.type != "VariableDeclarator") {
    ...
    if (
      ...
      if (
        astUtils.isSpecificMemberAccess(initialization.callee, "ChromeUtils", /^importESModule$/)
      ) {
        // Hack alert: our eslint env is pretty confused about `require` and
        // `loader` for devtools modules - so ignore it for now.
        // See bug 1812547
