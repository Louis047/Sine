/**
 * @file Reject common XPCOM methods called with useless optional parameters, or non-existent
 *   parameters.
 * @license
...
export default {
  meta: {
    ...
    messages: {
      ...
      obmittedWhenFalse: "{{fnName}}'s {{index}} parameter can be omitted when it's false.",
  ...
  create(context) {
    function getRangeAfterArgToEnd(argNumber, args) {
      ...
      return [sourceCode.getTokenAfter(args[argNumber]).range[0], args[args.length - 1].range[1]];
    }
    ...
    return {
      CallExpression(node) {
        ...
        let isBool = (arg) => arg.type === "Literal" && (arg.value === false || arg.value === true);
        let name = callee.property.name;
        let args = node.arguments;

        if (
          ["addEventListener", "removeEventListener", "addObserver"].includes(name) &&
          args.length === 3 &&
          isFalse(args[2])
        ) {
          context.report({
        ...
        if (name === "getComputedStyle" && args.length === 2 && isFalsy(args[1])) {
          context.report({
            node,
            fix: (fixer) => {
              return fixer.removeRange(getRangeAfterArgToEnd(0, args));
        ...
        if (name === "newURI" && args.length > 1 && isFalsy(args[args.length - 1])) {
          context.report({
            node,
            fix: (fixer) => {
