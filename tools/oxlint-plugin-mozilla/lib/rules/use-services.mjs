...
export default {
  meta: {
    ...
    // fixable: "code",
    messages: {
      useServices: "Use Services.{{ serviceName }} rather than {{ getterName }}.",
    },
    schema: [],
    type: "suggestion",
  },
    ...
    return {
      CallExpression(node) {
        ...
        if (
          ...
          let serviceName = servicesInterfaceMap[node.arguments[3].property.name];
        ...
        if (
          ...
          for (let property of node.arguments[1].properties) {
            if (
              ...
              let serviceName = servicesInterfaceMap[property.value.elements[1].property.name];
