/**
 * @file Reject uses of unknown interfaces on Ci and properties of those interfaces.
 * @license
...
export default {
  meta: {
    ...
    messages: {
      ...
      unknownProperty: "Use of unknown property Ci.{{ interface }}.{{ property }}",
    },
    schema: [],
    type: "problem",
  },
      ...
      MemberExpression(node) {
        if (
          ...
          if (!helpers.xpidlData.get(node.property.name)) {
            let platformSpecific = platformSpecificInterfaces.get(node.property.name);
            if (!platformSpecific) {
              context.report({
                node,
        ...
        if (
          ...
          if (!interfaceHasProperty(node.object.property.name, node.property.name)) {
            context.report({
              node,
