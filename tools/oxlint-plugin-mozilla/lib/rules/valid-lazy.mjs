/**
 * @file Ensures that definitions and uses of properties on the `lazy` object are valid.
 * @license
...
export default {
  ...
  create(context) {
    ...
    function isDuplicate(prevNode, currNode) {
      ...
      for (let i = 0; i < prevAncestors.length && i < currAncestors.length; i++) {
    ...
    function addProp(callNode, propNode, name) {
      if (lazyProperties.has(name) && isDuplicate(lazyProperties.get(name).callNode, callNode)) {
        context.report({
          node: propNode,
