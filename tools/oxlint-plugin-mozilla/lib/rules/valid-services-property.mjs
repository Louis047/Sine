/**
 * @file Ensures that property accesses on Services.<alias> are valid. Although this largely
 *   duplicates the valid-services rule, the checks here require an objdir and a manual run.
 * @license
    ...
    return {
      MemberExpression(node) {
        ...
        if (node.object.name == "Services") {
          mainNode = node;
        } else if (node.property.name == "Services" && node.parent.type == "MemberExpression") {
        ...
        if (mainNode.parent.type == "MemberExpression" && !mainNode.parent.computed) {
